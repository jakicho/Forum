...
public class HeaderLayoutManagerFixed extends RecyclerView.LayoutManager {
     ...
     * than
     * this factor times the total space of the list. If authorLayout is vertical, total space is the
     * height minus padding, if authorLayout is horizontal, total space is the width minus padding.
     */
    ...
    /**
     * Defines if authorLayout should be calculated from end to start.
     *
     ...
     * It is calculated by checking {@link #getReverseLayout()} and View's authorLayout direction.
     ...
     * authorLayout which will check this variable and re-authorLayout accordingly.
     */
     ...
     *                      #VERTICAL}.
     * @param reverseLayout When set to true, renders the authorLayout from end to start.
     */
    ...
    /**
     * Returns the current orientaion of the authorLayout.
     *
    ...
    /**
     * Sets the orientation of the authorLayout. {@link android.support.v7.widget.LinearLayoutManager}
    ...
    /**
     * Calculates the view authorLayout order. (e.g. from end to start or start to end)
     * RTL authorLayout support is applied automatically. So if authorLayout is RTL and
    ...
    /**
     * Returns if views are laid out from the opposite direction of the authorLayout.
     *
     * @return If authorLayout is reversed or not.
    ...
    /**
     * Used to reverse item traversal and authorLayout order.
     * This behaves similar to the authorLayout change for RTL views. When set to true, first item is
     * rendered at the end of the UI, second item is render before it etc.
     * <p/>
     * For horizontal layouts, it depends on the authorLayout direction.
     ...
     * custom authorLayout pre-cache logic.</p>
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        ensureRenderState();
        // resolve authorLayout direction
            ...
            if (mPendingSavedState != null) {
                ...
                if (child != null) {
                    ...
                    if (childSize > mOrientationHelper.getTotalSpace()) {
                        // item does not fit. fix depending on authorLayout direction
                ...
                // override authorLayout from end values for consistency
        ...
        if (getChildCount() > 0) {
            // because authorLayout from end may be changed by scroll to position
            // we re-calculate it.
        ...
        // If there are scrap children that we did not authorLayout, we need to find where they did go
        // and authorLayout them accordingly so that animations can work as expected.
     ...
     * <p/>
     * <p>Note that scroll position change will not be reflected until the next authorLayout call.</p>
     *
    ...
    /**
     * <p>Scroll to the specified adapter position with the given offset from authorLayout start.</p>
     * <p/>
     * <p>Note that scroll position change will not be reflected until the next authorLayout call.</p>
     * <p/>
    ...
    private void updateRenderState(int layoutDirection, int requiredSpace,
        ...
        if (layoutDirection == RenderState.LAYOUT_END) {
            ...
            // calculate how much we can scroll without adding new children (independent of authorLayout)
    ...
    /**
     * Recycles views that went out of bounds after scrolling towards the end of the authorLayout.
     *
        ...
        if (dt < 0) {
            if (DEBUG) {
                Log.d(TAG, "Called recycle from start with a negative value. This might happen"
                        + " during authorLayout changes but may be sign of a bug");
            }
            return;
        }
    ...
    /**
     * Recycles views that went out of bounds after scrolling towards the start of the authorLayout.
     *
     * @param recycler Recycler instance of {@link android.support.v7.widget.RecyclerView}
     * @param dt       This can be used to add additional padding to the visible area. This is used
    ...
    /**
     * Helper method to call appropriate recycle method depending on current render authorLayout
     * direction
     *
    ...
    /**
     * The magic functions :). Fills the given authorLayout, defined by the renderState. This is fairly
    ...
    private int fill(RecyclerView.Recycler recycler, RenderState renderState,
        ...
        while (remainingSpace > 0 && renderState.hasMore(state)) {
            ...
            // To calculate correct authorLayout position, we subtract margins.
     ...
     * children.
     *
     * @return The child closes to start of the authorLayout from user's perspective.
     */
     ...
     * children.
     *
     * @return The child closes to end of the authorLayout from user's perspective.
     */
     ...
     * <p/>
     * Note that, this value is not affected by authorLayout orientation or item order traversal.
     * ({@link #setReverseLayout(boolean)}). Views are sorted by their positions in the adapter,
     * not in the authorLayout.
     * <p/>
     ...
     * <p/>
     * In default authorLayout, child 0 should be closest to screen position 0 and last child should be
     * closest to position WIDTH or HEIGHT.
     * In reverse authorLayout, last child should be closes to screen position 0 and first child should
    ...
    private static class RenderState {
        ...
        /**
         * Number of pixels that we should fill, in the authorLayout direction.
         */
        int mAvailable;
        ...
        /**
         * Defines the direction in which the notifDataList adapter is traversed.
         * Should be {@link #ITEM_DIRECTION_HEAD} or {@link #ITEM_DIRECTION_TAIL}
         */
        int mItemDirection;

        /**
         * Defines the direction in which the authorLayout is filled.
        ...
        /**
         * Used if you want to pre-authorLayout items that are not yet visible.
        ...
        /**
         * When LLM needs to authorLayout particular views, it sets this list in which case, RenderState
         * will only return views from this list and return null if it cannot find an item.
         */
        List<RecyclerView.ViewHolder> mScrapList = null;

        /**
         * @return true if there are more items in the notifDataList adapter
         */
    ...
    static interface OrientationHelper {
         ...
         *
         * @param amount Value to add to each child's authorLayout parameters
         */
        void offsetChildren(int amount);

        /**
         * Returns the total space to authorLayout.
         *
         * @return Total space to authorLayout children
         */
        int getTotalSpace();
    }
